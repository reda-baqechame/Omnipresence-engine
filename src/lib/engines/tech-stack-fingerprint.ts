/**
 * Pure, dependency-free tech-stack fingerprint engine (no IO, no `@/` imports).
 *
 * Rule-based signatures matched against a page's HTML, headers, cookies and meta
 * tags — our keyless BuiltWith/Wappalyzer-lite. Kept separate from the network
 * fetch + OmniData routing (tech-stack.ts) so detection precision can be audited
 * directly against known-stack HTML fixtures.
 */
...
// Curated open fingerprint set covering the technologies that matter most for
// AEO/competitive intel. Patterns mirror public Wappalyzer-style signatures.
